...
void MultirateEKFhorLin::execute(void)
{
	...
	double psiRad = mPhiThetaPsiHat->psi*M_PI/180.;
	double cpsi=cos(psiRad);
	double spsi=sin(psiRad);
	double rRad = mpqrHat->r*M_PI/180.;
	//
	...
	// latitude-longitude
	double dxPos,dyPos;
	dxPos=mPosition->mPose.x; dyPos=mPosition->mPose.y;
	double xLat=xPred(0,0)+dxPos*cpsi-dyPos*spsi;
	double yLon=xPred(1,0)+dxPos*spsi+dyPos*cpsi;
	// NED velocity prediction
	double dxSpeed,dySpeed;
	dxSpeed=mSpeedNED->mPose.x; dySpeed=mSpeedNED->mPose.y;
	double xDotPred = xPred(2,0)*cpsi-xPred(3,0)*spsi+xPred(4,0)-rRad*(dxSpeed*spsi+dySpeed*cpsi);
	double yDotPred = xPred(2,0)*spsi+xPred(3,0)*cpsi+xPred(5,0)+rRad*(dxSpeed*cpsi-dySpeed*spsi);
	double Upred2=xDotPred*xDotPred+yDotPred*yDotPred;
	double Upred=sqrt(Upred2);
	double chiPred=atan2(yDotPred , xDotPred);
	//  surge-sway rate
	double dxVel,dyVel;
	dxVel=mBodyVelocity->mPose.x; dyVel=mBodyVelocity->mPose.y;
	double uPred = xPred(2,0)+xPred(4,0)*cpsi+xPred(5,0)*spsi-rRad*dyVel;
	double vPred = xPred(3,0)-xPred(4,0)*spsi+xPred(5,0)*cpsi+rRad*dxVel;
	...
	H(2,2) = (xPred(2,0)+xPred(4,0)*cpsi+xPred(5,0)*spsi-rRad*dySpeed)/Upred;
	H(2,3) = (xPred(3,0)-xPred(4,0)*spsi+xPred(5,0)*cpsi+rRad*dxSpeed)/Upred;
	H(2,4) = xDotPred/Upred;
	H(2,5) = yDotPred/Upred;
	H(3,2) = -(xPred(3,0)-xPred(4,0)*spsi+xPred(5,0)*cpsi+rRad*dxSpeed)/Upred2;
	H(3,3) = (xPred(2,0)+xPred(4,0)*cpsi+xPred(5,0)*spsi-rRad*dySpeed)/Upred2;
	...
	//
	bool flagPosition = mPosition->mValid;
	bool flagNEDspeed = mSpeedNED->mValid;
	bool flagVelocity = mBodyVelocity->mValid;
	...
	double xV,yV;
	lat_lon_2_x_y(xV,yV,*lat0,*lon0,mPosition->lat,mPosition->lon);
	z(0,0) = xV;
	z(1,0) = yV;
	z(2,0) = mSpeedNED->speed;
	z(3,0) = mSpeedNED->track*M_PI/180.;
	z(4,0) = mBodyVelocity->u;
	z(5,0) = mBodyVelocity->v;
	...
	R(0,0) = SQR(mPosition->latStd);
	R(1,1) = SQR(mPosition->lonStd);
	R(2,2) = SQR(mSpeedNED->speedStd);
	R(3,3) = SQR(mSpeedNED->trackStd*M_PI/180);
	R(4,4) = SQR(mBodyVelocity->uStd);
	R(5,5) = SQR(mBodyVelocity->vStd);
	...
	// update estimate
	//
	lat_lon_plus_x_y(mPositionHat->lat,mPositionHat->lon,*lat0,*lon0,x(0,0),x(1,0));
	mBodyVelocityWrtWaterHat->u = x(2,0);
	mBodyVelocityWrtWaterHat->v = x(3,0);
	mSeaCurrentHat->xCdot = x(4,0);
	mSeaCurrentHat->yCdot = x(5,0);

	mPositionHat->mValid = mBodyVelocityWrtWaterHat->mValid = mSeaCurrentHat->mValid = !mInitFlag;
}
...
{
//	printf("MultirateEKFhorLin::reset %s begin\n",mName.c_str()); fflush(stdout);
...
//	printf("MultirateEKFhorLin::reset %s end\n",mName.c_str()); fflush(stdout);
