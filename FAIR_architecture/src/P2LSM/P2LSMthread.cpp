...
//	printf("P2LSMthread::P2LSMthread %s %d %d\n",mName.c_str(),mPolicy,mPriority);fflush(stdout);
	mP2LSM = new P2LSM();
//	printf("P2LSMthread::P2LSMthread %s end\n",mName.c_str());fflush(stdout);
}
...
//	mP2LSM->set_sensor_info(mSensorInfo,&nSensor);
	...
	if(!mP2LSM->read_configuration()) {
		printf("P2LSMthread::init_configuration failed! %s\n",mName.c_str());fflush(stdout);
		return false;
	}
...
void P2LSMthread::process_command(char *cmd,ssize_t n) {
...
void P2LSMthread::build_data(void) {
	//
