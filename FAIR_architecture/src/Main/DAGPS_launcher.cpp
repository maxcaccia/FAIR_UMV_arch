...
//void process_packet(uint8_t* packet,char* peerIpAddr,uint16_t peerPort,UdpSocket* serviceUDP,GenericThread* thread,ThreadProperties thrProperties);
...
int main(int argc, char* argv[])
{
	...
	//
	if (argc < 4) {
		// <threadName> <config path> and <serviceUDPtimeOut> are always requested
		printf("Wrong parameter number\n\tmain_launcher_<module>  <threadName> <config path> <serviceUDPtimeOut> <true/false>\n");
		fflush(stdout);
		return -1;
	}
	...
	int serviceTimeout=stoi(argv[3]);
	...
	//	}
	//
	if(thrProperties.moduleName=="DAGPS") {
		genericThread = new DAGPSthread();
	}
	else {
		printf("ERROR: DAGPSthread not implemented!\n"); fflush(stdout);
		return 0;
	}
	//***** end insert your code *************************************
	//
	...
	//
	bool isActual=false;
	if((thrProperties.moduleClass=="DRIVER")&&(argc==5)) {
		string flag=argv[4];
		if(flag=="true")
			isActual=true;
		//***** insert your code here ************************************
		...
		//
		((DAGPSthread*)genericThread)->set_is_actual(isActual);
		//***** end insert your code *************************************
	}
	//
	// build thread comms
