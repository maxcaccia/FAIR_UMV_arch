...
int main(int argc, char* argv[])
{
	...
	//
	if (argc < 4) {
		// <threadName>, <serviceUDPtimeOut> and true/false are always requested
	...
	//	}
	//
	if(thrProperties.moduleName=="GNSS") {
		genericThread = new GNSSthread();
	}
	else {
		printf("ERROR: GNSSthread not implemented!\n"); fflush(stdout);
		return 0;
	}
	//***** end insert your code *************************************
	//
	...
	if((thrProperties.moduleClass=="DRIVER")&&(argc==4)) {
		...
		//
		((GNSSthread*)genericThread)->set_is_actual(isActual);
		//***** end insert your code *************************************
	}
	//
	// build thread comms
