...
int main(int argc, char* argv[])
{
	...
	if(thrProperties.moduleName=="MINION") {
		genericThread = new MinionUDPthread();
	}
	else {
		printf("ERROR: MinionUDPthread not implemented!\n"); fflush(stdout);
		return 0;
	}
	//***** end insert your code *************************************
	//
	...
	if((thrProperties.moduleClass=="DRIVER")&&(argc==4)) {
		...
		//
		((MinionUDPthread*)genericThread)->set_is_actual(isActual);
		//***** end insert your code *************************************
	}
	//
	// build thread comms
