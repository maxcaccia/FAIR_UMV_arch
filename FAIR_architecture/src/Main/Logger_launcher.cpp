...
//
// main
//	called as: main_launcher_<module> <threadName> <serviceUDPtimeOut>
//		where
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3) {
		...
		printf("Wrong parameter number\n\tmain_launcher_<module> <threadName> <serviceUDPtimeOut>\n");
		fflush(stdout);
		return -1;
	}
	...
	//
	if(thrProperties.moduleName=="LOGGER") {
		genericThread = new LoggerThread();
	}
	else {
		printf("ERROR: NGCthread not implemented!\n"); fflush(stdout);
		return 0;
	}
	...
	//
	genericThread->init_configuration();
	//
	// build thread comms
	//
...
//			printf("%d %d %s subscribe_to %s ipAddress %s port %d\n",i,thrProperties.subscribeToFlag[i],thrProperties.name.c_str(),thrProperties.subscribeToName[i].c_str(),
...
////printf("process_packet peerIpAddr %s peerPort %" PRIu16 "\n",peerIpAddr,peerPort);
//	uint16_t cmd;
//	uint16_t dataRecvPort;
//	uint8_t ack[64];
//	sscanf((const char*)packet,"%" SCNu16 " %" SCNu16,&cmd,&dataRecvPort);
//	string ipAddr=peerIpAddr;
//printf("process_packet cmd %d peerIpAddr %s peerPort %" PRIu16 "\n",cmd,peerIpAddr,peerPort);fflush(stdout);
//	switch(cmd) {
...
//				printf("%d %d %s subscription to %s %s %" PRIu16 " acknowledged\n",i,thrProperties.subscribeToFlag[i],
