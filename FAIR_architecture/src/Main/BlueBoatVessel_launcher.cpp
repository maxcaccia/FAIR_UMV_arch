...
//
// main
//	called as: <module>_launcher <threadName> <serviceUDPtimeOut> <true/false>
//		where
...
int main(int argc, char* argv[])
{
	...
	printf("\n%s serviceUDP binded at ipAddress %s port %d\n",
			thrProperties.name.c_str(),thrProperties.ipAddress.c_str(),thrProperties.serviceUdpPort);fflush(stdout);
	...
	//***** insert your code here ************************************
	//
	if(thrProperties.moduleName=="BLUEBOAT_SIM") {
		genericThread = new BlueBoatVesselThread();
	}
	else {
		printf("ERROR: BlueBoatVesselThread not implemented!\n"); fflush(stdout);
		return 0;
	}
	//***** end insert your code *************************************
	//
	...
	while(running) {
		...
		while((n=serviceUDP.recvfrom(packet, 64, 0,peerIpAddr,peerPort))!=-1) {
			printf("serviceUDP.recvfrom %s %s %" PRIu16 "\r\n",(char*)packet,peerIpAddr,peerPort);
			//
