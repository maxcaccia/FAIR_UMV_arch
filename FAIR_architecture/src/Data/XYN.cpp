...
XYN::XYN(string name):TimeStampedData(name) {
	mPacketCode=XYN_CODE;
	reset();
}

XYN::XYN(void):TimeStampedData() {
	mPacketCode=XYN_CODE;
	reset();
}
...
XYN & XYN::operator = (const XYN &obj) {
...
void XYN::sprint_csv_log_name(char *s) {
...
void XYN::sprint_csv_long_name(char *s) {
	char strTS[256],str[2048];
